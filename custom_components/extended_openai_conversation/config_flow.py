...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        schema = vol.Schema({
            ...
            vol.Optional(CONF_API_VERSION, default=""): str,  # Azure/compat only
            vol.Optional(CONF_ORGANIZATION, default=""): str,
            vol.Optional(CONF_CHAT_MODEL, default=DEFAULT_CHAT_MODEL): str,
            vol.Optional(CONF_MODEL_STRATEGY, default=DEFAULT_MODEL_STRATEGY): vol.In(
                [MODEL_STRATEGY_FORCE_CHAT, MODEL_STRATEGY_FORCE_RESPONSES, DEFAULT_MODEL_STRATEGY]
            ),
            vol.Optional(CONF_USE_RESPONSES_API, default=DEFAULT_USE_RESPONSES_API): bool,
            vol.Optional(CONF_REASONING_EFFORT, default=""): vol.In(REASONING_EFFORT_CHOICES),
        })
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        schema = vol.Schema({
            ...
            # Generation (temperature/top_p ignored for GPT‑5 reasoning models)
            ...
            vol.Optional(CONF_MAX_TOKENS, default=_opt(self.entry, CONF_MAX_TOKENS, DEFAULT_MAX_TOKENS)): vol.All(vol.Coerce(int), vol.Range(min=1)),
            vol.Optional(CONF_MAX_COMPLETION_TOKENS, default=_opt(self.entry, CONF_MAX_COMPLETION_TOKENS, 4096)): vol.All(vol.Coerce(int), vol.Range(min=1)),
            ...
            vol.Optional(CONF_STREAM_MIN_CHARS, default=_opt(self.entry, CONF_STREAM_MIN_CHARS, DEFAULT_STREAM_MIN_CHARS)): vol.All(vol.Coerce(int), vol.Range(min=0)),

            # Truncation / context
            vol.Optional(CONF_CONTEXT_THRESHOLD, default=_opt(self.entry, CONF_CONTEXT_THRESHOLD, DEFAULT_CONTEXT_THRESHOLD)): vol.All(vol.Coerce(int), vol.Range(min=0)),
            ...
            # Router (optional)
            vol.Optional(CONF_USE_TOOLS, default=_opt(self.entry, CONF_USE_TOOLS, DEFAULT_USE_TOOLS)): bool,
            ...
            vol.Optional(CONF_MAX_FUNCTION_CALLS_PER_CONVERSATION, default=_opt(self.entry, CONF_MAX_FUNCTION_CALLS_PER_CONVERSATION, DEFAULT_MAX_FUNCTION_CALLS_PER_CONVERSATION)): vol.All(vol.Coerce(int), vol.Range(min=0)),
        })
